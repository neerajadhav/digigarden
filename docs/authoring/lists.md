...
Zensical supports several flavors of lists that cater to different use cases,
including _unordered lists_ and _ordered lists_, which are supported through
standard Markdown, as well as _definition lists_ and _task lists_, which are
...
When [Definition Lists] is enabled, lists of arbitrary key-value pairs, e.g.,
the parameters of functions or modules, can be enumerated with a simple syntax:
